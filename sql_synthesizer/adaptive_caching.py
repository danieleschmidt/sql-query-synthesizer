...
class AdaptiveCacheManager:
    ...
    def __init__(
        ...
        self.access_patterns: Dict[str, List[float]] = defaultdict(
            list
        )  # key -> [timestamps]
    ...
    def should_cache(
        ...
        with self._lock:
            ...
            if len(access_history) >= self.hit_threshold_for_caching:
                ...
                now = time.time()
                recent_accesses = [
                    t for t in access_history if now - t < 3600
                ]  # Last hour
    ...
    def set(
        ...
        if not should_cache:
            logger.debug(
                f"Skipping cache for key {key[:12]}... (strategy: {strategy.value})"
            )
            return
    ...
    def _ensure_memory_capacity(self, needed_bytes: int):
        ...
        logger.info(
            f"Evicted {len(candidates[:self.eviction_count])} entries, freed {freed_memory} bytes"
        )
...
class PredictiveCacheWarmer:
    ...
    def predict_next_queries(self, lookahead_minutes: int = 30) -> List[str]:
        ...
        for query_hash, execution_times in self.prediction_history.items():
            ...
            recent_executions = [
                t for t in execution_times if now - t < 24 * 3600
            ]  # Last 24 hours
