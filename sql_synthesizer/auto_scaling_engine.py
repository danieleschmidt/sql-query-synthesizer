...
class MetricsCollector:
    ...
    def start_collection(self):
        ...
        self.running = True
        self._collector_task = threading.Thread(
...
class ScalingDecisionEngine:
    ...
    def _get_metric_for_trigger(
        self, trigger: ScalingTrigger
    ) -> Optional[ScalingMetric]:
    ...
    def _evaluate_rule(
        self, rule: ScalingRule, metric: ScalingMetric, current_time: float
        ...
        if (
            ...
            target_instances = min(
                int(current_instances * rule.scale_factor), rule.max_instances
            )
            ...
            target_instances = max(
                int(current_instances / rule.scale_factor), rule.min_instances
            )
    ...
    def _trend_supports_scaling(
        self, metric: ScalingMetric, direction: ScalingDirection
    ) -> bool:
    ...
    def get_scaling_statistics(self) -> Dict[str, Any]:
        ...
        execution_times = [
            a.execution_time for a in self.scaling_history if a.execution_time > 0
        ]
        ...
        return {
            ...
            "current_resource_levels": {
                k.value: v for k, v in self.current_resources.items()
            },
        }
        ...
        while self.running and not self._shutdown_event.is_set():
            try:
                ...
                for action in actions:
                    ...
                    if success:
                        logger.info(
                            f"Scaling action {action.action_id} completed successfully"
                        )
                    else:
