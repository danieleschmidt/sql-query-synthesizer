...
class SQLSecurityValidator:
    ...
    def __init__(self):
        ...
        self.dangerous_functions = {
            'exec', 'execute', 'sp_executesql', 'xp_cmdshell', 'openrowset',
            'opendatasource', 'openquery', 'openxml', 'sp_oacreate',
    ...
    def _ast_based_security_check(self, sql: str) -> List[ValidationIssue]:
        ...
        try:
            ...
            for token in parsed.flatten():
                if token.ttype is T.Name and token.value.lower() in self.dangerous_functions:
                    issues.append(ValidationIssue(
    ...
    def _semantic_security_check(self, sql: str) -> List[ValidationIssue]:
        ...
        if any(cmd in sql_lower for cmd in ['grant', 'revoke', 'create user', 'drop user']):
            issues.append(ValidationIssue(
    ...
    def _extract_table_names(self, sql: str) -> Set[str]:
        ...
        try:
            ...
            for i, token in enumerate(tokens):
                if token.ttype is T.Keyword and token.value.upper() in ('FROM', 'JOIN', 'UPDATE', 'INTO'):
...
class PerformanceValidator:
    ...
    def _create_performance_issue(self, issue_type: str, sql: str) -> ValidationIssue:
        ...
        issue_info = issues_map.get(issue_type, {
            'id': 'PERF_UNKNOWN',
            'severity': ValidationSeverity.INFO,
...
class BusinessRulesValidator:
    ...
    def validate_business_rules(self, sql: str, context: Dict[str, Any] = None) -> List[ValidationIssue]:
    ...
    def _check_table_policies(self, sql: str, context: Dict[str, Any]) -> List[ValidationIssue]:
...
class ComprehensiveValidator:
    ...
    def validate(self, sql: str, context: Dict[str, Any] = None) -> ValidationResult:
        ...
        business_issues = self.business_rules_validator.validate_business_rules(sql, context)
        all_issues.extend(business_issues)
        
        # Calculate scores
        security_score = self._calculate_score(security_issues, ValidationType.SECURITY)
        performance_score = self._calculate_score(performance_issues, ValidationType.PERFORMANCE)
    ...
    def _calculate_score(self, issues: List[ValidationIssue], validation_type: ValidationType) -> float:
    ...
    def _generate_recommendations(self, issues: List[ValidationIssue]) -> List[str]:
        ...
        if ValidationType.PERFORMANCE in by_type:
            ...
            recommendations.append("Consider query restructuring for better performance")
