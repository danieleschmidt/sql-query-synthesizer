...
class Config:
    ...
    def __init__(self, force_reload=False):
        ...
        self.webapp_port = self._get_int_env(
            "QUERY_AGENT_WEBAPP_PORT", 5000, min_value=1
        )
        self.webapp_input_size = self._get_int_env(
            "QUERY_AGENT_WEBAPP_INPUT_SIZE", 60, min_value=1
        )

        # Query Processing Limits
        self.max_question_length = self._get_int_env(
            "QUERY_AGENT_MAX_QUESTION_LENGTH", 1000, min_value=1
        )
        self.default_max_rows = self._get_int_env(
            "QUERY_AGENT_DEFAULT_MAX_ROWS", 5, min_value=1
        )

        # Cache Configuration
        self.cache_cleanup_interval = self._get_int_env(
            "QUERY_AGENT_CACHE_CLEANUP_INTERVAL", 300, min_value=1
        )
        self.cache_backend = (
            os.getenv("QUERY_AGENT_CACHE_BACKEND", "memory").lower().strip()
        )
        self.cache_ttl = self._get_int_env("QUERY_AGENT_CACHE_TTL", 3600, min_value=0)
        self.cache_max_size = self._get_int_env(
            "QUERY_AGENT_CACHE_MAX_SIZE", 1000, min_value=1
        )
        ...
        self.memcached_servers = self._get_list_env(
            "QUERY_AGENT_MEMCACHED_SERVERS", ["localhost:11211"]
        )

        # Timeout Settings (in seconds)
        self.openai_timeout = self._get_int_env(
            "QUERY_AGENT_OPENAI_TIMEOUT", 30, min_value=1
        )
        self.database_timeout = self._get_int_env(
            "QUERY_AGENT_DATABASE_TIMEOUT", 30, min_value=1
        )

        # Database Connection Pool Settings
        self.db_pool_size = self._get_int_env(
            "QUERY_AGENT_DB_POOL_SIZE", 10, min_value=1
        )
        self.db_max_overflow = self._get_int_env(
            "QUERY_AGENT_DB_MAX_OVERFLOW", 20, min_value=0
        )
        self.db_pool_recycle = self._get_int_env(
            "QUERY_AGENT_DB_POOL_RECYCLE", 3600, min_value=1
        )
        self.db_pool_pre_ping = self._get_bool_env("QUERY_AGENT_DB_POOL_PRE_PING", True)
        self.db_connect_retries = self._get_int_env(
            "QUERY_AGENT_DB_CONNECT_RETRIES", 3, min_value=0
        )
        self.db_retry_delay = self._get_float_env(
            "QUERY_AGENT_DB_RETRY_DELAY", 1.0, min_value=0.1
        )

        # Circuit Breaker Settings for LLM Provider Resilience
        self.circuit_breaker_failure_threshold = self._get_int_env(
            "QUERY_AGENT_CIRCUIT_BREAKER_FAILURE_THRESHOLD", 5, min_value=1
        )
        self.circuit_breaker_recovery_timeout = self._get_float_env(
            "QUERY_AGENT_CIRCUIT_BREAKER_RECOVERY_TIMEOUT", 60.0, min_value=1.0
        )

        # Enhanced SQL Injection Prevention
        self.use_enhanced_sql_validation = self._get_bool_env(
            "QUERY_AGENT_USE_ENHANCED_SQL_VALIDATION", True
        )

        # Pagination Configuration
        self.default_page_size = self._get_int_env(
            "QUERY_AGENT_DEFAULT_PAGE_SIZE", 10, min_value=1
        )
        self.max_page_size = self._get_int_env(
            "QUERY_AGENT_MAX_PAGE_SIZE", 1000, min_value=1
        )
        ...
        self.webapp_rate_limit = self._get_int_env(
            "QUERY_AGENT_RATE_LIMIT_PER_MINUTE", 60, min_value=1
        )
        self.webapp_enable_hsts = self._get_bool_env("QUERY_AGENT_ENABLE_HSTS", False)
        self.webapp_api_key_required = self._get_bool_env(
            "QUERY_AGENT_API_KEY_REQUIRED", False
        )
        self.webapp_api_key = os.environ.get("QUERY_AGENT_API_KEY", None)
        self.webapp_max_request_size = self._get_int_env(
            "QUERY_AGENT_MAX_REQUEST_SIZE_MB", 1, min_value=1
        )

        # Security Audit Logging Settings
        self.security_audit_enabled = self._get_bool_env(
            "QUERY_AGENT_SECURITY_AUDIT_ENABLED", True
        )
        self.security_log_level = os.environ.get(
            "QUERY_AGENT_SECURITY_LOG_LEVEL", "INFO"
        )
        self.security_log_format = os.environ.get(
            "QUERY_AGENT_SECURITY_LOG_FORMAT", "json"
        )
        self.audit_log_file = os.environ.get(
            "QUERY_AGENT_AUDIT_LOG_FILE", "/var/log/sql_synthesizer/audit.log"
        )
        self.audit_log_max_bytes = self._get_int_env(
            "QUERY_AGENT_AUDIT_LOG_MAX_BYTES", 50 * 1024 * 1024, min_value=1024
        )  # 50MB default
        self.audit_log_backup_count = self._get_int_env(
            "QUERY_AGENT_AUDIT_LOG_BACKUP_COUNT", 10, min_value=0
        )
        self.audit_log_retention_days = self._get_int_env(
            "QUERY_AGENT_AUDIT_LOG_RETENTION_DAYS", 90, min_value=1
        )
        ...
        self.database_query_buckets = self._get_bucket_env(
            "QUERY_AGENT_DATABASE_QUERY_BUCKETS", (0.1, 0.5, 1, 2, 5, 10, 30)
        )
    ...
    def _get_int_env(self, env_var: str, default: int, min_value: int = None) -> int:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid value for {env_var.lower().replace('query_agent_', '')}: '{value_str}' is not a valid integer"
            )
    ...
    def _get_bool_env(self, env_var: str, default: bool) -> bool:
        ...
        if value_lower in ("true", "1", "yes", "on"):
            ...
            config_name = env_var.lower().replace("query_agent_", "")
            raise ValueError(
                f"Invalid boolean value for {config_name}: '{value_str}' (use true/false)"
            )

    def _get_float_env(
        self, env_var: str, default: float, min_value: float = None
    ) -> float:
        ...
        try:
            ...
            config_name = env_var.lower().replace("query_agent_", "")
            raise ValueError(
                f"Invalid value for {config_name}: '{value_str}' is not a valid number"
            )
    ...
    def _get_bucket_env(
        self, env_var: str, default: Tuple[float, ...]
    ) -> Tuple[float, ...]:
