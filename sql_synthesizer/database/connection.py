...
class DatabaseManager:
        ...
        for attempt in range(self.config.connect_retries):
            try:
                ...
                if attempt < self.config.connect_retries - 1:
                    ...
                    logger.warning(
                        f"Connection test failed (attempt {attempt + 1}), retrying in {wait_time}s: {e}"
                    )
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(
                        f"Database connection test failed after {self.config.connect_retries} attempts: {e}"
                    )
                    raise
    ...
    async def get_connection_stats(self) -> ConnectionStats:
        ...
        try:
            ...
            health_score = max(
                0.0, 1.0 - connection_utilization * 0.5 - error_rate * 0.5
            )
