...
class QueryHistoryRepository(BaseRepository):
    ...
    async def delete(self, query_id: str) -> bool:
        ...
        try:
            ...
            rows_affected = await self.db_manager.execute_query(query, {'query_id': query_id})
    ...
    def _row_to_dict(self, row) -> Dict[str, Any]:
        ...
        columns = [
            'id', 'query_id', 'user_question', 'generated_sql', 
            'execution_time_ms', 'success', 'error_message', 
            'cache_hit', 'user_agent', 'client_ip', 'created_at', 'updated_at'
        ]
...
class SystemMetricsRepository(BaseRepository):
    ...
    async def get_by_id(self, metric_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await self.db_manager.execute_query(query, {'metric_id': metric_id})
    ...
    async def delete(self, metric_id: int) -> bool:
        ...
        try:
            ...
            rows_affected = await self.db_manager.execute_query(query, {'metric_id': metric_id})
            return rows_affected > 0
    ...
    async def get_metrics_by_name(self, metric_name: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = await self.db_manager.execute_query(
                query, 
                {'metric_name': metric_name, 'limit': limit}
            )
    ...
    def _row_to_dict(self, row) -> Dict[str, Any]:
        """Convert database row to dictionary."""
        columns = ['id', 'metric_name', 'metric_value', 'metric_type', 'tags', 'recorded_at']
        return {col: row[i] if i < len(row) else None for i, col in enumerate(columns)}