...
class QueryFeatureExtractor:
    ...
    def __init__(self):
        ...
        self.join_types = {"INNER", "LEFT", "RIGHT", "FULL", "CROSS"}
    ...
    def extract_features(self, sql: str) -> Dict[str, Any]:
        ...
        features = {
            ...
            "aggregate_usage": {
                func: tokens.count(func) for func in self.aggregate_functions
            },
            "join_types": {
                jtype: sql_upper.count(f"{jtype} JOIN") for jtype in self.join_types
            },
            # Complexity indicators
            "parentheses_depth": self._calculate_max_depth(sql, "(", ")"),
            "subquery_count": sql_upper.count("SELECT") - 1,  # Main query + subqueries
            "where_conditions": sql_upper.count(" AND ")
            + sql_upper.count(" OR ")
            + (1 if "WHERE" in sql_upper else 0),
            # Semantic features
            "has_aggregation": any(
                func in sql_upper for func in self.aggregate_functions
            ),
    ...
    def _extract_column_references(self, sql: str) -> List[str]:
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                columns.update(
                    [col for col in cols if col and not col.upper().startswith("FROM")]
                )
...
class PatternMiner:
    ...
    def discover_patterns(self, query_history: List[Dict]) -> List[QueryPattern]:
        ...
        if len(query_history) < self.min_frequency:
            logger.warning(
                f"Insufficient query history ({len(query_history)} queries) for pattern mining"
            )
            return patterns
        ...
        for query_data in query_history:
            ...
            features["timestamp"] = query_data.get(
            ...
            for group in groups:
                ...
                if group:
                    ...
                    if (
                        ...
                        and similarity > self.pattern_threshold
                    ):
    ...
    def _create_pattern_from_group(
        self, group_id: int, group_queries: List[Tuple]
        ...
        performance_times = [
            ...
            for _, features in group_queries
            if features["performance"] > 0
        ]
        ...
        pattern_id = (
            f"pattern_{group_id}_{hashlib.md5(description.encode()).hexdigest()[:8]}"
        )
        ...
        return QueryPattern(
            ...
            confidence=min(
                len(group_queries) / 10, 1.0
            ),  # Confidence based on frequency
            examples=[
                query_data.get("sql", "")[:200] + "..."
                for query_data, _ in group_queries[:3]
            ],
            created_at=datetime.utcnow().isoformat(),
            last_seen=max(
                features.get("timestamp", "") for _, features in group_queries
            ),
        )
...
class AdaptiveLearningEngine:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = (
            Path(storage_path) if storage_path else Path("./adaptive_learning_data")
        )
    ...
    def learn_from_queries(self, query_history: List[Dict]) -> List[QueryPattern]:
        ...
        try:
            ...
            logger.info(
                f"Learned {len(new_patterns)} new patterns, total patterns: {len(self.learned_patterns)}"
            )
            return new_patterns
    ...
    def generate_insights(
        self, current_sql: str, context: Optional[Dict] = None
        ...
        try:
            ...
            current_features = self.pattern_miner.feature_extractor.extract_features(
                current_sql
            )
            ...
            for pattern in self.learned_patterns:
                ...
                similarity_score = self._calculate_pattern_match(
            ...
            for pattern, similarity in matching_patterns[:5]:
                insight = self._create_insight_from_pattern(
                    pattern, similarity, current_features
                )
                if insight:
            ...
            performance_insights = self._generate_performance_insights(
                current_features, matching_patterns
            )
    ...
    def get_learning_statistics(self) -> Dict:
        ...
        avg_confidence = (
            np.mean([p.confidence for p in self.learned_patterns])
            if self.learned_patterns
            else 0
        )
        avg_success_rate = (
            np.mean([p.success_rate for p in self.learned_patterns])
            if self.learned_patterns
            else 0
        )
            ...
            "learning_age_days": (
                (
                    ...
                    - datetime.fromisoformat(
                        min(
                            [p.created_at for p in self.learned_patterns],
                ...
                for i, existing_pattern in enumerate(self.learned_patterns):
                    if existing_pattern.pattern_id == new_pattern.pattern_id:
                        ...
                        self.learned_patterns[i].avg_performance_ms = (
                            ...
                            + new_pattern.frequency * new_pattern.avg_performance_ms
                        ) / total_freq
                        break
            else:
                # Add new pattern
    ...
    def _calculate_pattern_match(
        self, current_features: Dict, pattern: QueryPattern
    ) -> float:
        ...
        if pattern.description:
            ...
            if "aggregation" in description_lower and current_features.get(
                "has_aggregation"
            ):
                score += 0.3
            ...
            if "subquery" in description_lower and current_features.get("has_subquery"):
                score += 0.3
            if (
                "complex" in description_lower
                and current_features.get("token_count", 0) > 50
            ):
                score += 0.2

        return min(score, 1.0)

    def _create_insight_from_pattern(
        self, pattern: QueryPattern, similarity: float, current_features: Dict
    ...
    def _generate_performance_insights(
        self, current_features: Dict, matching_patterns: List[Tuple]
    ) -> List[LearningInsight]:
        ...
        avg_perf = np.mean(
            [pattern.avg_performance_ms for pattern, _ in matching_patterns]
        )
        ...
        if avg_perf > 2000:  # 2 seconds
            insights.append(
                LearningInsight(
                    ...
                    supporting_evidence=[
                        f"Based on {len(matching_patterns)} similar patterns"
                    ],
