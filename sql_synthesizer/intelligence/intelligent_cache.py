...
class SemanticCacheIndex:
    ...
    def find_similar_queries(self, sql: str, features: Dict[str, Any], 
...
class AccessPatternPredictor:
    ...
    def predict_next_access(self, query_hash: str) -> Tuple[float, float]:
        ...
        with self.lock:
            ...
            intervals = [recent_history[i] - recent_history[i-1] 
            ...
            interval_std = np.std(intervals) if len(intervals) > 1 else avg_interval * 0.5
...
class IntelligentCacheManager:
    ...
    def put(self, key: str, value: Any, context: Optional[QueryContext] = None,
            ttl: Optional[int] = None, estimated_cost: float = 1.0):
        ...
        with self.lock:
            ...
            if context and hasattr(context, 'query_features'):
                self.semantic_index.add_query(
                    entry.query_hash, 
                    key, 
                    getattr(context, 'query_features', {})
                )
        ...
        with self.lock:
            ...
            if hit_rate < 0.3:
                insights.append(CacheInsight(
                    ...
                    confidence=0.9,
                    supporting_data={'current_hit_rate': hit_rate, 'cache_size': len(self.cache)}
                ))
            ...
            if utilization > 0.9:
                insights.append(CacheInsight(
                    ...
                    confidence=0.9,
                    supporting_data={'utilization': utilization, 'evictions': self.stats['evictions']}
                ))
        ...
        with self.lock:
            ...
            return {
                ...
                'semantic_caching': {
                    ...
                    'semantic_hit_rate': self.stats['semantic_hits'] / max(total_requests, 1),
                },
                'predictive_caching': {
                    'predictive_hits': self.stats['predictive_hits'],
                    'predictive_hit_rate': self.stats['predictive_hits'] / max(total_requests, 1),
                },
                'capacity': {
                ...
                'entries': {
                    ...
                    'avg_age_seconds': np.mean([time.time() - e.created_at for e in self.cache.values()]) if self.cache else 0,
                }
            }
    ...
    def _semantic_lookup(self, key: str, context: QueryContext) -> Optional[Any]:
        ...
        for query_hash, similarity in similar_queries:
            ...
            for entry_key, entry in self.cache.items():
                if (entry.query_hash == query_hash and 
                    self._is_valid_entry(entry) and
                    similarity >= self.min_confidence_threshold):
    ...
    def _predictive_lookup(self, key: str, context: Optional[QueryContext]) -> Optional[Any]:
    ...
    def _select_eviction_victim(self) -> Optional[str]:
        ...
        for key, entry in self.cache.items():
            ...
            access_probability, _ = self.pattern_predictor.predict_next_access(query_hash)
    ...
    def _matches_invalidation_pattern(self, key: str, entry: CacheEntry, pattern: str) -> bool:
    ...
    def _analyze_access_patterns(self) -> List[CacheInsight]:
        ...
        for entry in self.cache.values():
            if entry.access_pattern:
                recent_pattern = [ts for ts in entry.access_pattern if time.time() - ts < 3600]
                if recent_pattern:
