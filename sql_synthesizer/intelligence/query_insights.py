...
class QueryPatternAnalyzer:
    ...
    def _create_pattern_insight(
        self, pattern_name: str, sql: str
    ) -> Optional[QueryInsight]:
...
class QueryComplexityAnalyzer:
    ...
    def analyze_complexity(self, sql: str) -> QueryComplexity:
        ...
        # Count joins
        join_count = len(
            re.findall(r"\b(?:INNER|LEFT|RIGHT|FULL|CROSS)\s+JOIN\b", sql_upper)
        )
        ...
        agg_functions = len(
            re.findall(r"\b(?:COUNT|SUM|AVG|MAX|MIN|GROUP_CONCAT)\s*\(", sql_upper)
        )

        # Count WHERE conditions (approximation)
        where_conditions = len(re.findall(r"\bAND\b|\bOR\b", sql_upper)) + (
    ...
    def _count_subquery_depth(self, sql: str) -> int:
        ...
        for i, char in enumerate(sql):
            ...
            if char == "(":
                ...
                if any(
                    keyword in prev_context
                    for keyword in ["SELECT", "FROM", "WHERE", "HAVING"]
                ):
                    depth += 1
...
class QueryInsightsEngine:
    ...
    def analyze_query(
        self, sql: str, execution_time_ms: Optional[float] = None
    ) -> Dict:
        ...
        try:
            ...
            if execution_time_ms:
                performance_insights = self._analyze_performance(
                    sql, execution_time_ms, complexity
                )
    ...
    def _analyze_performance(
        self, sql: str, execution_time_ms: float, complexity: QueryComplexity
    ) -> List[QueryInsight]:
    ...
    def batch_analyze_queries(self, queries: List[Dict]) -> Dict:
        ...
        for query_data in queries:
            ...
            if analysis.get("complexity"):
                aggregate_stats["avg_complexity"] += analysis["complexity"][
                    "total_score"
                ]
