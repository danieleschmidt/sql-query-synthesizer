...
class LoadBalancer:
    ...
    def _filter_candidates(
        ...
        for endpoint in self.endpoints:
            ...
            if is_read_only:
                ...
                if endpoint.role in [DatabaseRole.PRIMARY, DatabaseRole.REPLICA, DatabaseRole.ANALYTICS]:
    ...
    def _apply_load_balancing(
        self, 
        candidates: List[DatabaseEndpoint], 
        estimated_cost: float
    ) -> Optional[DatabaseEndpoint]:
    ...
    def update_endpoint_stats(
        ...
        for endpoint in self.endpoints:
            if endpoint.name == endpoint_name:
                endpoint.response_time_ms = stats['total_response_time'] / stats['total_requests']
                if not success:
    ...
    async def health_check_endpoints(self):
        ...
        for endpoint in self.endpoints:
            try:
                ...
                stats = self._endpoint_stats[endpoint.name]
                recent_requests = max(1, stats['total_requests'] - endpoint.current_load * 100)
                endpoint.current_load = min(recent_requests / endpoint.max_connections, 1.0)
...
class IntelligentQueryRouter:
    ...
    def __init__(self):
        ...
        self.routing_history: List[Tuple[str, QueryRoute, float]] = []  # (query_hash, route, execution_time)
    ...
    async def route_query(self, sql: str, context: Dict[str, Any] = None) -> QueryRoute:
        ...
        # Select endpoint
        endpoint = self.load_balancer.select_endpoint(complexity, is_read_only, estimated_cost)
        
        if not endpoint:
            raise Exception("No available database endpoints for query routing")
        
        # Determine caching strategy
        cache_strategy = self._determine_cache_strategy(complexity, estimated_cost, context)
        
        # Create routing decision
        route = QueryRoute(
            endpoint=endpoint,
            reason=f"Selected {endpoint.role.value} for {complexity.value} query (cost: {estimated_cost:.1f})",
            estimated_performance=100 - estimated_cost,  # Higher performance = lower cost
        ...
        async with self._lock:
            self.routing_history.append((query_hash, route, 0))  # execution_time will be updated later
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy_endpoints": sum(1 for e in self.load_balancer.endpoints if e.healthy)
        }
