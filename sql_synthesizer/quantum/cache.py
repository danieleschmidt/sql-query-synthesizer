...
from .core import QueryPlan, QuantumState
...
class QuantumPlanCache:
    ...
    def __init__(self, 
        ...
        self._performance_history: List[Tuple[str, float, float]] = []  # (key, cost_reduction, success)
    ...
    def put(self, key: str, plan: QueryPlan, ttl: Optional[float] = None,
            cost_reduction: float = 0.0, metadata: Dict[str, Any] = None) -> bool:
        """
        Store plan in cache
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._performance_history:
                avg_cost_reduction = sum(cr for _, cr, _ in self._performance_history) / len(self._performance_history)
            else:
    ...
    def get_top_entries(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            entries_with_scores = [
                (entry, entry.calculate_value_score())
    ...
    def _evict_entry(self) -> bool:
        ...
        if self.strategy == CacheStrategy.LRU:
            ...
            key = min(self._cache.keys(), 
                     key=lambda k: self._cache[k].calculate_value_score())
...
class QuantumCacheManager:
    ...
    def put(self, key: str, plan: QueryPlan, cost_reduction: float = 0.0,
        ...
        if cost_reduction > 0.1:  # 10% improvement
            self.l2_cache.put(key, plan, cost_reduction=cost_reduction, metadata=metadata)
        
        # Store in L3 if very high value
        if cost_reduction > 0.3:  # 30% improvement
            self.l3_cache.put(key, plan, cost_reduction=cost_reduction, metadata=metadata)
    ...
    def get_combined_stats(self) -> Dict[str, Any]:
        ...
        total_requests = sum(stats[f"l{i}_cache"]["total_requests"] for i in range(1, 4))
