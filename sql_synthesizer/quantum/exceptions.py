...
class QuantumErrorRecovery:
    ...
    @staticmethod
    def should_retry(
        error: QuantumError, attempt_count: int = 0, max_attempts: int = 3
    ) -> bool:
        ...
        no_retry_types = {"QuantumValidationError", "QuantumSecurityError"}
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type and issubclass(exc_type, QuantumError):
            ...
            if self.logger:
                ...
                self.logger.error(
                    f"Quantum operation '{self.operation_name}' failed",
                    extra={
            ...
            if self.enable_recovery:
                ...
                if should_retry:
                    if self.logger:
                        retry_delay = QuantumErrorRecovery.get_retry_delay(
                            self.attempt_count
                        )
                        self.logger.info(
                            f"Retrying quantum operation '{self.operation_name}' "
                ...
                if fallback and self.logger:
                    self.logger.info(
                        f"Quantum operation '{self.operation_name}' "
