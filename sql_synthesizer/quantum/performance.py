...
class QuantumPerformanceMonitor:
    ...
    def record_metric(self, metric: PerformanceMetrics):
        ...
        with self._lock:
            ...
            if operation_type not in self._metrics:
                self._metrics[operation_type] = collections.deque(
    ...
    def _calculate_stats_from_metrics(
    ...
    def _adapt_performance(self):
        ...
        with self._lock:
            ...
            speed_score = max(
                0, 1.0 - avg_exec_time / self._thresholds["max_execution_time"]
            )
            reliability_score = max(
                0, 1.0 - p95_exec_time / (self._thresholds["max_execution_time"] * 2)
            )

            self._performance_score = (
                success_score * 0.5 + speed_score * 0.3 + reliability_score * 0.2
            )
                ...
                base_params.update(
                    {
                        ...
                        "parallel_workers": min(
                            base_params.get("parallel_workers", 4), 2
                        ),
                ...
                base_params.update(
                    {
                        ...
                        "batch_size": max(
                            5, int(base_params.get("batch_size", 10) * score)
                        ),
                        "parallel_workers": max(
                            2, int(base_params.get("parallel_workers", 4) * score)
                        ),
                    }
                )
    ...
    def _get_base_parameters(self, operation_type: str) -> Dict[str, Any]:
        ...
        if operation_type == "quantum_optimization":
            base_params.update(
                {"batch_size": 20, "parallel_workers": 8, "iterations": 1000}
            )
        elif operation_type == "quantum_scheduling":
            base_params.update(
                {"batch_size": 50, "parallel_workers": 6, "scheduling_window": 300}
            )
        elif operation_type == "plan_generation":
            base_params.update(
                {"batch_size": 100, "parallel_workers": 12, "max_plans": 50}
            )
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if success_rate < 0.95:
            recommendations.append(
                "Consider switching to conservative mode for better reliability"
            )
        ...
        if avg_exec_time > 20.0:
            recommendations.append(
                "Consider increasing parallel workers or reducing batch sizes"
            )

        if p95_exec_time > 45.0:
            recommendations.append(
                "Implement timeout optimizations and circuit breakers"
            )
        ...
        if memory_stats and memory_stats.get("max", 0) > 0.8:
            recommendations.append(
                "Memory usage is high - consider implementing memory pooling"
            )

        # Mode-specific recommendations
        if (
            self._current_mode == OptimizationMode.CONSERVATIVE
            and self._performance_score > 0.9
        ):
            recommendations.append(
                "Performance is good - consider switching to balanced mode"
            )

        elif self._current_mode == OptimizationMode.AGGRESSIVE and success_rate < 0.95:
            recommendations.append(
                "Aggressive mode causing failures - consider balanced mode"
            )
        ...
        return {
            **self._pool_stats,
            ...
            "optimizer_reuse_rate": (
                ...
                / max(
                    1,
                    self._pool_stats["optimizers_created"]
                    + self._pool_stats["optimizers_reused"],
                )
            ),
            "scheduler_reuse_rate": (
                ...
                / max(
                    1,
                    self._pool_stats["schedulers_created"]
                    + self._pool_stats["schedulers_reused"],
                )
            ),
        }
