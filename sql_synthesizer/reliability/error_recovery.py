...
class ErrorClassifier:
    ...
    ) -> Tuple[str, ErrorSeverity, RecoveryStrategy]:
    ...
    async def retry_with_backoff_async(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                delay = self._calculate_delay(attempt)
                logger.warning(
                    f"Async retry attempt {attempt + 1}/{self.max_retries} after {delay:.2f}s: {e}"
                )
...
class ErrorRecoveryManager:
    ...
    def handle_error(
        ...
        # Classify error
        error_category, severity, recovery_strategy = (
            self.error_classifier.classify_error(error, context)
        )
        ...
        try:
            ...
            logger.error(
                f"Recovery failed for error {error_context.error_id}: {recovery_error}"
            )
    ...
    def register_recovery_action(self, error_category: str, action: RecoveryAction):
        ...
        logger.info(
            f"Registered recovery action for {error_category}: {action.strategy}"
        )
    ...
    def get_error_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recovery_stats": {
                ...
                "recovery_rate": (
                    self.recovery_stats["recovered_errors"] / max(total_errors, 1)
                )
                * 100,
            },
    ...
    def _retry_recovery(
        self, error_context: ErrorContext, original_error: Exception
    ) -> Any:
        ...
        if recovery_action and recovery_action.action_function:
            return self.retry_manager.retry_with_backoff(
                recovery_action.action_function, error_context=error_context
            )
        else:
    ...
    def _circuit_breaker_recovery(self, error_context: ErrorContext) -> Any:
        ...
        if breaker.state == "OPEN":
            ...
            raise CircuitBreakerOpenError(
                f"Circuit breaker prevents operation on {component}"
            )

    def _graceful_degradation(self, error_context: ErrorContext) -> Any:
        """Graceful degradation recovery."""
        logger.warning(
            f"Graceful degradation applied for critical error {error_context.error_id}"
        )
