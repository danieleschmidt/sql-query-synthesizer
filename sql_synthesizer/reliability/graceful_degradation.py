...
class GracefulDegradationManager:
    ...
    def register_capability(self, capability: ServiceCapability):
        ...
        with self.lock:
            ...
            logger.info(f"Registered service capability: {capability.name} (priority: {capability.priority})")
    
    def trigger_degradation(self, level: DegradationLevel, reason: str, 
                          affected_capabilities: Optional[List[str]] = None):
        """Manually trigger service degradation."""
        with self.lock:
            if level.value == self.current_level.value and affected_capabilities is None:
            ...
            degraded_capabilities = self._apply_degradation_level(level, affected_capabilities)
        ...
        with self.lock:
            ...
            if memory_percent > self.triggers['memory_threshold']:
                ...
                if memory_percent > 95:
                    suggested_level = max(suggested_level, DegradationLevel.CRITICAL_DEGRADATION, key=lambda x: list(DegradationLevel).index(x))
                elif memory_percent > 92:
                    suggested_level = max(suggested_level, DegradationLevel.MAJOR_DEGRADATION, key=lambda x: list(DegradationLevel).index(x))
            ...
            if recent_errors > self.triggers['error_rate_threshold']:
                ...
                if recent_errors > 50:
                    ...
                    suggested_level = max(suggested_level, DegradationLevel.MINOR_DEGRADATION, key=lambda x: list(DegradationLevel).index(x))
            
            # Auto-trigger if needed
            if degradation_needed and suggested_level != self.current_level:
                self.trigger_degradation(
                    suggested_level,
                    f"Auto-triggered: {'; '.join(reasons)}",
                    None
                )
                return True
    ...
    def get_capability_status(self, capability_name: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'degraded_since': capability.degraded_since.isoformat() if capability.degraded_since else None,
    ...
    def get_degradation_status(self) -> Dict[str, Any]:
        ...
        with self.lock:
            enabled_capabilities = [name for name, cap in self.capabilities.items() if cap.enabled]
            degraded_capabilities = [name for name, cap in self.capabilities.items() if not cap.enabled]
    ...
    def execute_with_degradation(self, capability_name: str, 
        ...
        if capability.enabled:
            ...
            if capability.fallback_function:
                ...
                logger.warning(f"Capability {capability_name} unavailable and no fallback defined")
    ...
    def _apply_degradation_level(self, level: DegradationLevel, 
                               specific_capabilities: Optional[List[str]] = None) -> List[str]:
        ...
        if specific_capabilities:
            ...
            for cap_name in specific_capabilities:
                if cap_name in self.capabilities:
                    ...
                    self.capabilities[cap_name].degradation_reason = f"Specific degradation: {level.value}"
    ...
    def _check_recovery_conditions(self) -> bool:
        ...
        for capability in self.capabilities.values():
            if (not capability.enabled and 
                capability.degraded_since and 
                datetime.utcnow() - capability.degraded_since < timedelta(minutes=2)):
    ...
    def _calculate_degradation_level_from_capabilities(self) -> DegradationLevel:
        ...
        disabled_capabilities = [cap for cap in self.capabilities.values() if not cap.enabled]
    ...
    def _get_degradation_start_time(self) -> Optional[str]:
        ...
        for capability in self.capabilities.values():
            if not capability.enabled and capability.degraded_since:
                if earliest_degradation is None or capability.degraded_since < earliest_degradation:
