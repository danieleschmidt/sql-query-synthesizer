...
class HealthMonitor:
    ...
    def register_check(self, check: HealthCheck):
        ...
        with self.lock:
            ...
            logger.info(f"Registered health check '{check.name}' for component '{self.component_name}'")
    ...
    def get_overall_health(self) -> ComponentHealth:
        ...
        with self.lock:
            ...
            return ComponentHealth(
                ...
                message=message,
                timestamp=datetime.utcnow(),
                response_time_ms=sum(r.response_time_ms for r in self.results.values()) / len(self.results)
            )
    ...
    def _update_health_tracking(self, check_name: str, health: ComponentHealth, success: bool):
...
class SystemHealthManager:
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        for component_name, monitor in self.component_monitors.items():
            ...
            component_health[component_name] = {
                ...
                'checks': {name: asdict(result) for name, result in component_results.items()},
            ...
            if component_overall.status == HealthStatus.CRITICAL:
                overall_status = HealthStatus.CRITICAL
            elif component_overall.status == HealthStatus.UNHEALTHY and overall_status != HealthStatus.CRITICAL:
                overall_status = HealthStatus.UNHEALTHY
            elif component_overall.status == HealthStatus.DEGRADED and overall_status == HealthStatus.HEALTHY:
                overall_status = HealthStatus.DEGRADED
        ...
        if performance_health['status'] == HealthStatus.CRITICAL.value:
            overall_status = HealthStatus.CRITICAL
        elif (performance_health['status'] == HealthStatus.DEGRADED.value and 
              overall_status == HealthStatus.HEALTHY):
            overall_status = HealthStatus.DEGRADED
        ...
        health_report = {
            ...
            'summary': self._generate_health_summary(component_health, performance_health)
        }
    ...
    def _generate_health_summary(self, component_health: Dict, performance_health: Dict) -> str:
        ...
        if healthy_components == total_components and performance_health['status'] == 'healthy':
    ...
    def _send_alert(self, alert_type: str, health_report: Dict):
        ...
        logger.warning(f"HEALTH ALERT [{alert_type.upper()}]: {health_report['summary']}")
