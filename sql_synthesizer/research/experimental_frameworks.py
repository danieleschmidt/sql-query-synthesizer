...
class BaseExperimentalApproach(ABC):
    ...
    @abstractmethod
    async def synthesize_sql(self, natural_language: str, schema_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_execution_time_ms': sum(self.performance_metrics['execution_time']) / len(self.performance_metrics['execution_time']),
            'avg_accuracy_score': sum(self.performance_metrics['accuracy']) / len(self.performance_metrics['accuracy']),
            'avg_quality_score': sum(self.performance_metrics['quality']) / len(self.performance_metrics['quality']),
            'success_rate': len([r for r in self.execution_history if r.error_rate < 0.1]) / len(self.execution_history) * 100,
            'latest_result': asdict(self.execution_history[-1]) if self.execution_history else None
        }
...
class TemplateBasedApproach(BaseExperimentalApproach):
    ...
    def _extract_entities(self, natural_language: str, schema_context: Dict[str, Any]) -> Dict[str, Any]:
...
class SemanticSimilarityApproach(BaseExperimentalApproach):
    ...
    def get_approach_metadata(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'strengths': ['Learns from examples', 'Handles variations well', 'Improves over time'],
    ...
    def add_pattern(self, natural_language: str, sql: str, metadata: Optional[Dict] = None):
    ...
    def _initialize_knowledge_base(self):
        ...
        examples = [
            ...
            ("get the top 10 products by sales", "SELECT * FROM products ORDER BY sales DESC LIMIT 10"),
            ("show average order value by month", "SELECT DATE_TRUNC('month', created_at) as month, AVG(total) FROM orders GROUP BY month"),
            ("list customers with their order counts", "SELECT c.name, COUNT(o.id) FROM customers c LEFT JOIN orders o ON c.id = o.customer_id GROUP BY c.id, c.name")
        ]
...
class HybridApproach(BaseExperimentalApproach):
    ...
    async def synthesize_sql(self, natural_language: str, schema_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            template_result = await self.template_approach.synthesize_sql(natural_language, schema_context)
            similarity_result = await self.similarity_approach.synthesize_sql(natural_language, schema_context)
            
            # Weighted voting for final decision
            template_score = template_result.get('confidence', 0) * self.voting_weights['template_based']
            similarity_score = similarity_result.get('confidence', 0) * self.voting_weights['semantic_similarity']
            ...
            return {
                ...
                'confidence': max(template_result.get('confidence', 0), similarity_result.get('confidence', 0)),
                'approach': self.approach_name,
    ...
    def get_approach_metadata(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'strengths': ['Best of both approaches', 'Adaptive selection', 'Robust performance'],
...
class ExperimentalFramework:
    ...
    async def run_comparative_experiment(self, 
                                       ...
                                       approaches_to_test: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"Starting comparative experiment {experiment_id} with {len(test_cases)} test cases")
            ...
            for i, test_case in enumerate(test_cases):
                ...
                logger.debug(f"Running test case {i+1}/{len(test_cases)}: {natural_language[:50]}...")
                ...
                for approach_name in approaches_to_test:
                    ...
                    try:
                        # Run the approach
                        result = await approach.synthesize_sql(natural_language, schema_context)
                        ...
                        exp_result = ExperimentResult(
                            ...
                            metadata={
                                ...
                                'approach_specific': {k: v for k, v in result.items() if k not in ['sql', 'confidence', 'execution_time_ms']}
                            },
                        ...
                        logger.error(f"Error running approach {approach_name} on test case {i}: {e}")
    ...
    async def test_hypothesis(self, hypothesis_id: str, test_cases: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        hypothesis_results = self._analyze_hypothesis_results(hypothesis, experiment_results)
        ...
        return {
            ...
            'statistical_significance': hypothesis_results.get('statistical_significance', False)
        }
    ...
    def _evaluate_result_quality(self, result: Dict[str, Any], test_case: Dict[str, Any]) -> float:
        ...
        if expected_patterns:
            ...
            pattern_matches = sum(1 for pattern in expected_patterns if pattern.lower() in sql)
    ...
    def _analyze_experiment_results(self, experiment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for approach_name, results in results_by_approach.items():
            ...
            stats = {
                ...
                'success_rate': sum(1 for r in results if r.error_rate < 0.1) / len(results) * 100,
        ...
        def overall_score(stats):
            return (stats['avg_accuracy_score'] * 0.4 + 
        ...
        ranked_approaches = sorted(
            ...
            key=lambda x: overall_score(x[1]),
            reverse=True
        )
        ...
        if ranked_approaches:
            ...
            if len(ranked_approaches) > 1:
                ...
                improvement = overall_score(winner_stats) - overall_score(runner_up_stats)
                if improvement > 0.1:
                    analysis['insights'].append(f"Clear winner: {ranked_approaches[0][0]} outperforms others by {improvement:.1%}")
                else:
                    analysis['insights'].append("Close competition: Results are very similar across approaches")
            
            if winner_stats['avg_execution_time_ms'] < 100:
                analysis['insights'].append("Winner has excellent performance with sub-100ms execution time")
            
            if winner_stats['success_rate'] > 95:
                analysis['insights'].append("Winner demonstrates high reliability with >95% success rate")
        
        return analysis
    
    def _analyze_hypothesis_results(self, hypothesis: ResearchHypothesis, experiment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        baseline_metrics = {
            'avg_accuracy': sum(r.accuracy_score for r in baseline_results) / len(baseline_results),
            'avg_execution_time': sum(r.execution_time_ms for r in baseline_results) / len(baseline_results),
            'success_rate': sum(1 for r in baseline_results if r.error_rate < 0.1) / len(baseline_results)
        }
        
        novel_metrics = {
            'avg_accuracy': sum(r.accuracy_score for r in novel_results) / len(novel_results),
            'avg_execution_time': sum(r.execution_time_ms for r in novel_results) / len(novel_results),
            'success_rate': sum(1 for r in novel_results if r.error_rate < 0.1) / len(novel_results)
        }
        ...
        for criterion, expected_improvement in hypothesis.success_criteria.items():
            if criterion in baseline_metrics and criterion in novel_metrics:
                ...
                if criterion == 'avg_execution_time':
                    ...
                    improvement = (novel_value - baseline_value) / baseline_value if baseline_value > 0 else 0
        ...
        statistical_significance = len(baseline_results) >= 10 and len(novel_results) >= 10
        ...
        return {
            ...
            'confidence_level': hypothesis.confidence_level if hypothesis_validated else 0.0
        }
    ...
    def _generate_approach_summary(self, comparison: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'approach_types': list(set(p['type'] for p in performance_data if p['type'] != 'unknown')),
            'complexity_distribution': Counter(p['complexity'] for p in performance_data if p['complexity'] != 'unknown'),
            'performance_summary': performance_data,
            'research_recommendations': self._generate_research_recommendations(performance_data)
        }
    
    def _generate_research_recommendations(self, performance_data: List[Dict[str, Any]]) -> List[str]:
        ...
        if approach_types.most_common(1):
            ...
            if dominant_type != 'hybrid':
                recommendations.append(f"Explore hybrid approaches combining {dominant_type} with other methods")
        
        # Complexity analysis
        complexity_dist = Counter(p['complexity'] for p in performance_data)
        if complexity_dist.get('High', 0) > complexity_dist.get('Low', 0):
            recommendations.append("Investigate whether complexity benefits justify costs")
