...
class RobustErrorHandler:
    ...
    def _attempt_recovery(self, error_context: ErrorContext, original_exception: Exception):
        """Attempt to recover from an error using registered strategies."""
        
        strategies = self.recovery_strategies.get(error_context.category, [])
        
        if not strategies:
            logger.debug(f"No recovery strategies for category: {error_context.category.value}")
            return
        ...
        for strategy in strategies:
            ...
            try:
                ...
                success = self._execute_recovery_with_retry(strategy, error_context, original_exception)
        ...
        for attempt in range(strategy.max_attempts):
            try:
                ...
                if strategy.exponential_backoff and attempt > 0:
                    ...
                    logger.debug(f"Recovery attempt {attempt + 1}, waiting {delay:.2f}s")
                    time.sleep(delay)
                
                # Execute recovery function if provided
                if strategy.recovery_function:
                    result = strategy.recovery_function(error_context, original_exception)
                    if result:
                        return True
                ...
                if attempt == strategy.max_attempts - 1:
                    ...
                    if strategy.fallback_function:
                        try:
                            strategy.fallback_function(error_context, original_exception)
                            return True
    ...
    def _check_alerting_threshold(self, category: ErrorCategory):
        ...
        recent_errors = [
            ...
            if (e.category == category and 
                time.time() - e.timestamp < self.error_aggregation_window)
        ]
    ...
    def _get_most_common_errors(self, limit: int) -> List[Dict[str, Any]]:
        ...
        for error in self.error_history:
            ...
            error_types[error_type]['latest_message'] = error.metadata.get('exception_message', '')
        ...
        return [
            {
                ...
                'latest_message': data['latest_message'][:100] + '...' if len(data['latest_message']) > 100 else data['latest_message']
            }
...
class GracefulDegradation:
    ...
    def register_fallback(
        ...
        logger.info(f"Registered fallback for {service_name} at level {degradation_level}")
    ...
    def execute_with_degradation(self, service_name: str, primary_func: Callable, *args, **kwargs):
        ...
        for level in sorted(fallbacks.keys(), reverse=True):
            if level <= degradation_level:
                try:
                    logger.info(f"Using degraded service {service_name} at level {level}")
                    return fallbacks[level](*args, **kwargs)
                except Exception as e:
                    logger.error(f"Fallback level {level} failed for {service_name}: {e}")
                    continue
