...
class ConnectionPool:
    ...
    async def start(self):
        ...
        async with self.lock:
            ...
            logger.info(f"Connection pool '{self.pool_name}' started with {len(self.connections)} connections")
    ...
    @asynccontextmanager
    async def acquire(self, timeout: Optional[float] = None) -> AsyncContextManager[Any]:
        ...
        async with self.condition:
            while True:
                ...
                if remaining_time <= 0:
                    ...
                    raise asyncio.TimeoutError(f"Could not acquire connection from pool '{self.pool_name}' within {timeout}s")
                
                try:
                    await asyncio.wait_for(self.condition.wait(), timeout=remaining_time)
    ...
    async def _create_new_connection(self) -> Optional[PooledConnection]:
        ...
        try:
            ...
            if self.async_factory:
                ...
                raw_connection = await loop.run_in_executor(None, self.connection_factory)
    ...
    async def return_connection(self, connection: Any):
        ...
        async with self.condition:
            ...
            if (not self._running or 
                pooled_connection.error_count > 5 or
                not pooled_connection.health_check_passed):
                await self._close_connection(connection_id)
            else:
    ...
    async def _validate_connection(self, connection: PooledConnection) -> bool:
        ...
        try:
            ...
            healthy = await self.health_checker.check_connection_health(connection.connection)
    ...
    async def _cleanup_old_connections(self):
        ...
        for connection_id, connection in self.connections.items():
            ...
            if (age.total_seconds() > self.config.pool_recycle_seconds or
    ...
    def _update_avg_wait_time(self, wait_time_ms: float):
        ...
        if total_requests > 1:
            self.stats['avg_wait_time_ms'] = (current_avg * (total_requests - 1) + wait_time_ms) / total_requests
        else:
