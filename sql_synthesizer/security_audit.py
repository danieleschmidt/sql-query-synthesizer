...
class SecurityAuditLogger:
    ...
    def _setup_handlers(self) -> None:
        ...
        if self.config and hasattr(self.config, "audit_log_file"):
            try:
                ...
                max_bytes = getattr(
                    self.config, "audit_log_max_bytes", 50 * 1024 * 1024
                )
                ...
                if (
                    hasattr(self.config, "security_log_format")
                    and self.config.security_log_format.lower() == "json"
                ):
    ...
    def _cleanup_old_logs(self) -> None:
        ...
        if not (
            self.config
            and hasattr(self.config, "audit_log_file")
            and hasattr(self.config, "audit_log_retention_days")
        ):
            return
    ...
    def get_event_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics_since": datetime.now(timezone.utc)
            .isoformat()
            .replace("+00:00", "Z"),
        }
