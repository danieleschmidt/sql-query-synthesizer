...
class AsyncSQLGeneratorService:
    ...
    async def generate_sql(
        ...
        **kwargs: Any
        ...
        try:
            ...
            logger.info(f"Generated SQL via {self.llm_provider.get_provider_name()}: {sql[:100]}...")
            return sql
            
        except ProviderError as e:
            logger.warning(f"LLM provider failed, falling back to naive SQL: {e}")
            metrics.record_sql_generation_fallback(self.llm_provider.get_provider_name())
            return self._generate_naive_sql(question, available_tables)
    ...
    def _generate_naive_sql(self, question: str, available_tables: Optional[List[str]] = None) -> str:
