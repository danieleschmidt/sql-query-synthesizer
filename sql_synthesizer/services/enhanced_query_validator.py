...
COMPILED_TAUTOLOGY_PATTERNS = [
    re.compile(pattern, re.IGNORECASE) for pattern in TAUTOLOGY_PATTERNS
]
...
class EnhancedQueryValidatorService:
    ...
    def __init__(
        ...
        logger.info(
            f"Enhanced SQL injection prevention initialized with {len(COMPILED_ENHANCED_PATTERNS)} patterns"
        )
    ...
    def _contains_sql_injection(self, text: str) -> bool:
        ...
        for pattern in COMPILED_ENHANCED_PATTERNS:
            if pattern.search(text):
                logger.warning(
                    f"Enhanced SQL injection pattern detected: {pattern.pattern[:50]}..."
                )
                return True
        ...
        for pattern in COMPILED_TAUTOLOGY_PATTERNS:
            if pattern.search(text):
                logger.warning(
                    f"Tautology injection pattern detected: {pattern.pattern}"
                )
                return True
    ...
    def _contains_unauthorized_references(self, text: str) -> bool:
        ...
        if self.allowed_tables:
            ...
            is_sql_statement = any(
                re.search(pattern, text_lower) for pattern in sql_structure_patterns
            )
                ...
                for pattern in sql_table_patterns:
                    ...
                    for table in matches:
                        if table not in self.allowed_tables:
                            logger.warning(
                                f"Unauthorized table reference detected: {table}"
                            )
                            return True
    ...
    def _contains_suspicious_patterns(self, text: str) -> bool:
        ...
        for combo in suspicious_combinations:
            if all(word in text_lower for word in combo):
                logger.warning(
                    f"Suspicious keyword combination detected: {' + '.join(combo)}"
                )
                return True
    ...
    def _check_rate_limit(self, client_id: str) -> None:
        ...
        self.validation_attempts[client_id] = [
            timestamp
            for timestamp in self.validation_attempts[client_id]
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "allowed_tables_count": (
                len(self.allowed_tables) if self.allowed_tables else None
            ),
            "allowed_columns_count": (
                len(self.allowed_columns) if self.allowed_columns else None
            ),
