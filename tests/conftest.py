...
def mock_database_manager():
    ...
    mock_manager.get_connection_stats = AsyncMock(return_value=Mock(
        pool_size=10,
        checked_out=2,
        overflow=0,
        checked_in=8,
...
def mock_llm_provider():
    ...
    mock_provider = Mock()
    mock_provider.generate_sql = AsyncMock(return_value={
        'sql': 'SELECT * FROM users LIMIT 10;',
...
def mock_security_validator():
    ...
    mock_validator = Mock()
    mock_validator.validate_question = Mock(return_value={'is_valid': True, 'reason': None})
...
def mock_migration_manager():
    ...
    mock_manager.get_migration_status = AsyncMock(return_value={
...
def mock_repositories():
    ...
    query_repo.get_query_statistics = AsyncMock(return_value={
        'total_queries': 100,
    ...
    return {
        'query_history': query_repo,
        'system_metrics': metrics_repo
    }
...
def sample_api_request():
    ...
    return {
        'question': 'Show me the top 10 users by registration date',
        'max_rows': 10
    }
