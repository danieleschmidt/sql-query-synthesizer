...
class TestAsyncOpenAIAdapter:
    ...
    def setup_method(self):
        ...
        with patch(
            "sql_synthesizer.async_openai_adapter.AsyncOpenAI"
        ) as mock_client_class:
            mock_client_class.return_value = self.mock_openai
            self.adapter = AsyncOpenAIAdapter(
                api_key=self.api_key, model=self.model, timeout=30.0
            )
    ...
    async def test_async_generate_sql_success(self):
        ...
        result = await self.adapter.generate_sql(
            "Show me users older than 25", available_tables=["users", "orders"]
        )
        ...
        assert (
            "Show me users older than 25" in call_args.kwargs["messages"][0]["content"]
        )
    ...
    async def test_async_generate_sql_timeout_error(self):
        ...
        self.mock_openai.chat.completions.create.side_effect = MockTimeoutError(
            "Request timed out"
        )
    ...
    async def test_async_generate_sql_auth_error(self):
        ...
        self.mock_openai.chat.completions.create.side_effect = MockAuthError(
            "Invalid API key"
        )

        # Verify auth error is properly handled
        with pytest.raises(
            ProviderAuthenticationError, match="OpenAI authentication failed"
        ):
            await self.adapter.generate_sql("Test question")
    ...
    async def test_async_custom_timeout_configuration(self):
        ...
        with patch(
            ...
            custom_adapter = AsyncOpenAIAdapter(
                api_key=self.api_key, model=self.model, timeout=60.0
            )
