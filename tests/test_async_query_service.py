...
from sql_synthesizer.types import QueryResult, PaginationInfo
...
class TestAsyncQueryService:
    ...
    def setup_method(self):
        ...
        self.mock_context_manager.__aenter__ = AsyncMock(return_value=self.mock_connection)
        ...
        self.mock_transaction_manager.__aenter__ = AsyncMock(return_value=self.mock_connection)
        ...
        self.mock_inspector.get_table_names = AsyncMock(return_value=["users", "orders"])
    ...
    async def test_async_execute_sql(self):
        ...
        # Execute async SQL
        result = await self.async_query_service.execute_sql("SELECT COUNT(*) FROM users")
