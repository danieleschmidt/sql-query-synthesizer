...
def mock_engine():
    ...
    with patch("sql_synthesizer.database.create_engine") as mock_create, patch(
        "sql_synthesizer.database.event"
    ) as mock_event:
        mock_engine = Mock()
        ...
        with patch("sql_synthesizer.query_agent.inspect") as mock_inspect_agent, patch(
            "sql_synthesizer.services.query_service.inspect"
        ) as mock_inspect_service:
...
def test_error_counter_metrics():
    ...
    assert "validation_error_type" in str(
        metrics.INPUT_VALIDATION_ERRORS_TOTAL._labelnames
    )
