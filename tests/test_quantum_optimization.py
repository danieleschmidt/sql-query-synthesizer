...
from sql_synthesizer.quantum.core import (
    QuantumQueryOptimizer, QuantumQueryPlanGenerator, QueryPlan, 
    QuantumState, Qubit
)
...
class TestQuantumQueryOptimizer:
    ...
    def test_quantum_interference(self):
        ...
        optimizer = QuantumQueryOptimizer()
        plans = [
            QueryPlan([], [], [], 10.0, 0.5),  # Low cost - should get higher probability
            QueryPlan([], [], [], 50.0, 0.5)   # High cost - should get lower probability
        ]
...
class TestQuantumTaskScheduler:
    ...
    def test_find_ready_tasks(self):
        ...
        blocked_task = QuantumTask("blocked", TaskPriority.EXCITED_1, 30.0, ["missing_dep"])
        scheduler.task_queue.append(blocked_task)
        
        # Add unblocked task (dependency completed)
        unblocked_task = QuantumTask("unblocked", TaskPriority.EXCITED_1, 40.0, ["completed"])
...
class TestQuantumIntegration:
    ...
    async def test_concurrent_quantum_optimization(self):
        ...
        plans_sets = [
            [QueryPlan([], [], [], 10.0 + i, 0.0) for i in range(3)]
            for _ in range(5)
        ]
        
        # Run concurrent optimizations
        tasks = [
            optimizer.optimize_query_async(plans)
            for plans in plans_sets
        ]
