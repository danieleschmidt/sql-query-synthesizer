...
from sql_synthesizer.security_audit import security_audit_logger, SecurityEventType, SecurityEventSeverity
...
class TestSecurityAuditIntegration:
    ...
    def test_rate_limiting_violation_logs_audit_event(self):
        ...
        with patch('sql_synthesizer.services.enhanced_query_validator.security_audit_logger', self.mock_audit_logger):
            ...
            for _ in range(10):  # Exceed the low limit
                try:
                    validator.validate_question("SELECT * FROM users", client_id=client_id)
    ...
    def test_semantic_analysis_detection_logs_audit_event(self):
        ...
        assert (self.mock_audit_logger.log_sql_injection_attempt.called or 
                self.mock_audit_logger.log_event.called)
