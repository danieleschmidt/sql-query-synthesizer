...
from sql_synthesizer.utils.validators import (
    validate_query_length, validate_table_name, sanitize_input,
    is_safe_sql_identifier, contains_sql_injection_pattern,
    validate_column_list, validate_order_by_clause, validate_limit_clause
)
from sql_synthesizer.utils.helpers import (
    format_duration, truncate_string, safe_json_dumps, extract_client_info,
    sanitize_error_message, normalize_sql, parse_database_url,
    calculate_cache_key, validate_pagination_params
)
...
class TestValidators:
    ...
    def test_contains_sql_injection_pattern_basic(self):
        ...
        assert contains_sql_injection_pattern("UNION SELECT password FROM users") is not None
